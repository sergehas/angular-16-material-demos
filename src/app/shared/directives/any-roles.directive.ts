...
export class AnyRolesDirective implements OnInit {
  @Input("roles-any") roles: string[] | Role[] | undefined = [];
  ...
  ngOnInit() {
    if (!this.roles || this.roles.length === 0) {
      console.log(`[roles-any] no roles provided [${this.roles}]: grant access`);
      this.granted.emit(true);
      return;
    }
    const hasAccess = this._loginService.getLoggedUser().hasAnyRoles(this.roles as Role[]);
    console.log(`[roles-any] has access for roles [${this.roles}]: ${hasAccess}`);
