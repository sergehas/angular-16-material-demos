...
export class AnyRolesDirective implements OnInit {
  ...
  ngOnInit() {
    if (!this.roles || this.roles.length === 0) {
      console.log(
        `[appAnyRoles] no roles provided [${this.roles}]: grant access`
      );
      this.granted.emit(true);
      return;
    }
    const hasAccess = this._loginService
      .getLoggedUser()
      .hasAnyRoles(this.roles as Role[]);
    console.log(
      `[appAnyRoles] has access for roles [${this.roles}]: ${hasAccess}`
    );
